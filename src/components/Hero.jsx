    ...
    <section className="relative min-h-[100svh] flex items-center justify-center overflow-hidden bg-ink">
        ...
        <div className="flex flex-col items-start gap-8">
          ...
          <motion.div {...fadeUp(0)} className="flex flex-wrap gap-2">
            {profile.roles.map((role) => (
              <span
                key={role}
                className="px-3.5 py-1.5 rounded-full border border-paper/15 bg-paper/[0.04] backdrop-blur-sm font-mono text-[11px] tracking-[0.15em] uppercase text-paper/70"
              >
                {role}
              </span>
            ))}
          ...
          <motion.h1
            {...fadeUp(0.12)}
            className="font-display text-paper text-[clamp(2.8rem,11vw,7rem)] leading-[0.92] tracking-tight"
          >
            {profile.name}
          </motion.h1>
          ...
          <motion.p
            {...fadeUp(0.24)}
            className="max-w-lg font-sans text-paper/65 text-lg md:text-xl leading-relaxed"
          >
            {profile.tagline}
          </motion.p>
          ...
          <motion.div {...fadeUp(0.36)} className="flex flex-wrap gap-4 mt-2">
            <Link
              to="/gallery"
              className="group inline-flex items-center gap-2 px-7 py-3.5 bg-brush text-paper font-sans text-sm font-medium rounded-full hover:bg-brush-soft transition-all duration-300"
            >
              Eserlere göz at
      ...
      >
        ...
        <motion.div
          ...
          className="w-[1px] h-8 bg-gradient-to-b from-paper/30 to-transparent"
        />
      </motion.div>
    </section>
  );
